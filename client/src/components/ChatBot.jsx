...
//                   <div key={idx} className={bubbleClass(msg)}>
//                     {msg.answer}
//                     {msg.showtimes.map((s, i) => (
//                       <div key={i} className="text-xs mb-1 p-1 border rounded">
//                         {new Date(s.showDateTime).toLocaleString()} — Price: ₹{s.price}
//                       </div>
//                     ))}
//                   </div>
//                 );
...
//   const [open, setOpen] = useState(false);
//   const [messages, setMessages] = useState([
//     { sender: "bot", answer: "Hi! I can help you with movies, showtimes, and bookings. Ask me about currently playing movies to get started." }
//   ]);
//   const [input, setInput] = useState("");
//   const [loading, setLoading] = useState(false);
//   const messagesEndRef = useRef(null);
...
//   const baseUrl = (import.meta.env.VITE_BASE_URL || "").replace(/\/$/, "");
//   const userId = "user1"; // Replace with actual user ID if needed
...
//     if (!text) return;
//     pushMessage({ sender: "user", answer: text });
//     setInput("");
//     setLoading(true);

//     try {
//       const res = await fetch(`${baseUrl}/api/chat/interactive`, {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ userId, message: text }),
//       });
//       const data = await res.json();
//       pushMessage({ sender: "bot", answer: data.answer });
//     } catch (err) {
//       console.error(err);
//       pushMessage({ sender: "bot", answer: "Error reaching server. Please try again." });
//     } finally {
//       setLoading(false);
//     }
//   };
...
//               onChange={(e) => setInput(e.target.value)}
//               onKeyDown={(e) => e.key === "Enter" && sendMessage()}
//               placeholder="Ask about movies, showtimes, or book seats..."
//               className="flex-1 border rounded px-2 py-1 text-black"
//             />
//             <button
//               onClick={sendMessage}
...
export default function ChatBot() {
  const [open, setOpen] = useState(false);
  const [messages, setMessages] = useState([
    { sender: "bot", content: "Hi! I can help you with movies, showtimes, and bookings. Ask me about currently playing movies to get started." }
  ]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const messagesEndRef = useRef(null);
  ...
  const baseUrl = (import.meta.env.VITE_BASE_URL || "").replace(/\/$/, "");
  const userId = "user1"; // replace with real user ID if needed
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data.type === "movies" && data.movies) {
        const movieMessages = data.movies.map(m => ({
          sender: "bot",
          content: (
            <div className="flex gap-2 items-center">
              ...
              <div>
                ...
                <p>Showtime: {new Date(m.showDateTime).toLocaleString()}</p>
                <p>Rating: {m.rating} ⭐ | Runtime: {m.runtime} mins</p>
                <button
                  onClick={() => window.location.href = `/booking/${m.id}`}
        ...
        movieMessages.forEach(msg => pushMessage(msg));
      } 
      // Handle movieInfo
        ...
        pushMessage({
          ...
          content: (
            <div className="flex gap-2 items-start">
              <img src={m.poster} alt={m.title} className="w-20 h-28 object-cover rounded"/>
              <div>
